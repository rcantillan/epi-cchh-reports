...
```{r}
tabla_cobertura <- table_programa_cobertura(PROGRAMAS, cohorte, facultad, programa)

if (is.null(tabla_cobertura)) {
  cat("**Atención:** no se encontró el programa en PROGRAMAS (ver 01_load_data.R).")
} else {
  tabla_cobertura

  prog_row <- program_row(PROGRAMAS, cohorte, facultad, programa)
  nota_cautela <- build_caution_note(dplyr::first(prog_row$n_total))
  if (!is.null(nota_cautela)) {
    cat("\n\n> ", nota_cautela)
  }
}
```
...
```{r}
tabla_desglose <- table_programa_desglose(DESGLOSE, cohorte, facultad, programa)
if (is.null(tabla_desglose)) {
  cat("No hay datos de desglose para este programa.")
} else {
  tabla_desglose
}
```
