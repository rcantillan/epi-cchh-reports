...
tabla_rubrica_eval <- tibble::tibble(
  ...
  Descripción = c(
    "Integra enfoques teóricos y contextualización adecuada; comprensión analítica completa.",
    "Logro parcial con reconocimiento adecuado, pero con integración/contextualización aún parcial.",
    "Evidencia aprendizajes en desarrollo; requiere andamiaje para consolidar relaciones y comprensión.",
    "No identifica elementos fundamentales; impide relaciones o comprensiones mínimas.",
    "No se observa el desempeño por ausencia de evidencia.",
    "Respuesta no registrada."
  )
)
```

## Marco institucional y propósito del EPI

:::{.div style="font-size: 30px; text-align: left; margin-top: 3.0em; line-height: 1.35;"}
- **La Evaluación de Perfil Intermedio (EPI)** es un **hito formativo institucional** orientado a **monitorear trayectorias formativas** al término del **ciclo básico**.
- Su propósito central es **identificar el grado de desarrollo de competencias declaradas en el perfil intermedio** y distinguir entre:
  - **Estudiantes que avanzan con autonomía**, y
  - **Estudiantes que requieren apoyo formativo**.
- En el **Modelo Pedagógico institucional**, los **perfiles (ingreso, intermedio, egreso)** operan como **hitos evaluativos**, mientras los **ciclos formativos** concentran las **acciones de apoyo** derivadas de los resultados.

:::

## Perfil intermedio: sentido formativo

:::{.div style="font-size: 30px; text-align: left; margin-top: 2.6em; line-height: 1.35;"}
- El **perfil intermedio** define las **competencias** que deben estar desarrolladas al finalizar el **ciclo básico**.
- Es importante destacar que la ebvaluación de perfil intermedio **No** corresponde a una evaluación anticipada del **perfil de egreso**.
- Es una evaluación **compleja, integradora y formativa**, cuyos resultados orientan:

  - **continuidad con autonomía**, o
  - **continuidad con apoyos formativos**, según el nivel evidenciado.
...
tabla_rubrica_inst2 <- tabla_rubrica_inst %>%
  # 1) Categoría analítica (2 grandes grupos)
  dplyr::mutate(
    `Categoría analítica` = dplyr::case_when(
  ...
  dplyr::arrange(
    factor(`Categoría analítica`, levels = c(CAT_AUT, CAT_APO)),
    dplyr::desc(nivel_ord)
  ) %>%
  # 4) Mostrar la categoría solo una vez por bloque
  dplyr::mutate(
    `Categoría analítica` = dplyr::if_else(
      duplicated(`Categoría analítica`), "", `Categoría analítica`
    )
  ) %>%
  # 5) Dejar SOLO las columnas finales (elimina nivel_ord)
  dplyr::select(`Categoría analítica`, Nivel, `% Referencia`, Descripción)
...
tabla_rubrica_inst2 %>%
  ...
  # ✅ Anchos: dale espacio real a "Descripción"
...
## Rúbrica específica de la evaluación
...
if (!exists("tabla_rubrica_eval") || is.null(tabla_rubrica_eval) || nrow(tabla_rubrica_eval) == 0) {
  ...
  tabla_rubrica_eval2 <- tabla_rubrica_eval %>%
    dplyr::mutate(
      ...
      # Categoría analítica (dos bloques)
      `Categoría analítica` = dplyr::case_when(
        Nivel %in% c("4","3") ~ CAT_AUT,
        TRUE ~ CAT_APO
      )
    ) %>%
    # Orden: primero autonomía (4,3), luego apoyo (2,1,0,NR)
    dplyr::arrange(
      factor(`Categoría analítica`, levels = c(CAT_AUT, CAT_APO)),
      dplyr::desc(nivel_ord)
    ) %>%
    ...
    dplyr::select(`Categoría analítica`, Nivel, Categoría, Descripción)
...
- **Interpretación principal:** se realiza desde la **rúbrica institucional (referencial)**.
- **Operacionalización:** se implementa mediante la **rúbrica específica aplicada** en esta evaluación.
- Toda la lectura se estructura en torno a dos categorías analíticas (definiciones completas):
  - **Estudiantes que avanzan con autonomía**
  - **Estudiantes que requieren apoyo formativo**
...
if (nrow(final_items) > 0) {
  ...
  heat_df <- purrr::map_dfr(1:nrow(final_items), function(i) {
    ...
    tibble(raw = x) %>%
      mutate(
        ...
        level = if_else(is.na(num), NA_character_, as.character(num))
      ) %>%
      filter(level %in% keep_levels) %>%
      count(level, name = "n") %>%
  ...
  # ✅ Re-escalado por cuantiles (mejor contraste visual)
  qs <- as.numeric(stats::quantile(heat_df$prop, probs = c(0, .25, .50, .70, .85, 1), na.rm = TRUE))
  # evitar duplicados si hay muchos empates
  qs <- unique(qs)
  vals <- scales::rescale(qs)
  # si quedara muy corto por empates, fuerza un set mínimo
  if (length(vals) < 3) vals <- scales::rescale(c(0, 0.5, 1))

  # Colores semáforo (usa tu verde institucional)
  pal <- c(
    "#b71c1c",   # rojo fuerte
    "#ef5350",   # rojo
    "#f9a825",   # amarillo
    "#81c784",   # verde claro
    brand_main   # verde institucional
  )

  g <- ggplot(heat_df, aes(x = level, y = item, fill = prop)) +
    ...
    geom_text(
      aes(
        ...
        # texto oscuro cuando el fondo es claro (amarillo/verde), blanco cuando es rojo
        color = prop >= 0.55
      ),
      size = 7,
      fontface = "bold"
    ) +
    scale_color_manual(values = c("TRUE" = "#0b1f1f", "FALSE" = "white"), guide = "none") +
    # ✅ categorías abajo
    scale_x_discrete(position = "bottom", labels = nivel_labels_heat) +
    labs(x = NULL, y = NULL) +
    theme_void(base_size = 24) +
    theme(
      axis.text.x = element_text(
        size = 18, color = brand_text,
        lineheight = 1.05, margin = margin(t = 15)
      ),
      # ✅ eje Y pegado a la derecha
...
if (exists("final_items") && nrow(final_items) > 0) {
  ...
  if (nrow(ranking) == 0) {
    ...
    cat("## \n\n")
    cat("### Síntesis orientada a decisiones\n\n")
    ...
    cat(glue::glue("### Fortalezas (mayor proporción de {CAT_AUT})\n\n"))
    ...
    cat('::: {.column width="50%" style="font-size: 28px; text-align: left; margin-top: 2.0em;"}\n\n')
    cat(glue::glue("### Prioridades de apoyo (menor proporción de {CAT_AUT})\n\n"))
    ...
    cat("**Cierre interpretativo:** Este resumen integra la comparación entre ambos grupos y orienta la priorización de apoyos del ciclo formativo. ")
    cat("Para cada ítem, la composición interna permite decidir si el foco es **consolidación (nivel 2)** o **refuerzo de base (niveles 0–1)**.\n")
    cat(":::\n")
  }
}
```
