...
'), "\n\n")
    ...
    g <- ggplot(df, aes(x = level, y = n)) +
      ...
      theme(
        ...
        axis.text.x = element_text(size = 15, color = brand_text, lineheight = 1.1, margin = margin(t=10)),
...
if (nrow(final_items) > 0) {
  ...
  all_data <- purrr::map_dfr(1:nrow(final_items), function(i) {
    ...
    comp_id <- stringr::str_sub(code, 1, 1)
    comp_label <- paste("Competencia", comp_id)
    
    # LIMPIEZA DE ETIQUETA:
    # 1. Quitamos el número inicial (ej: "1.1")
    # 2. Usamos str_wrap para que el texto baje de línea y NO se corte
    item_label_clean <- str_replace(raw_title, "^\\d+(\\.\\d+)?(\\.[a-z])?\\s*", "")
    item_label <- stringr::str_wrap(item_label_clean, width = 45) # <--- AQUÍ ESTÁ EL CAMBIO CLAVE
    ...
    tibble(raw = x) %>%
      mutate(
        ...
        categoria = dplyr::case_when(
          num %in% 0:1 ~ "Brecha (Nivel 0-1)",
          num %in% 2:3 ~ "Logro (Nivel 2-3)",
          TRUE ~ NA_character_ 
        )
      ) %>%
  ...
  cat("\n\n---\n\n")
  cat("## Balance General por Competencia\n\n")
  
  # Ordenar
  all_data <- all_data %>%
    arrange(desc(code_order)) %>% 
  ...
  g <- ggplot(all_data, aes(x = item, y = prop, fill = categoria)) +
    ...
    # Agrupación visual
    facet_grid(competencia ~ ., scales = "free_y", space = "free_y", switch = "y") +
    
    scale_fill_manual(values = c(
      "Brecha (Nivel 0-1)" = "#d9534f", 
      "Logro (Nivel 2-3)"  = "#0a6d7c"
    )) +
    ...
    labs(
      ...
      #subtitle = "Porcentaje de estudiantes en niveles de Logro vs Brecha"
    ) +
    ...
    theme(
      ...
      # AJUSTE DE EJE Y (TEXTO DE LOS ÍTEMS)
      # Reducimos a size 11 y bajamos el interlineado (0.9) para que los textos largos se vean compactos
      axis.text.y = element_text(size = 11, color = "#333333", margin = margin(r=10), lineheight = 0.9),
      
      # Estilo de los títulos de las Competencias
      strip.text.y.left = element_text(angle = 0, face = "bold", size = 12, color = "#0a4e58"),
...
if (nrow(final_items) > 0) {
  ...
  all_data <- purrr::map_dfr(1:nrow(final_items), function(i) {
    ...
    # Limpieza y WRAP
    item_label_clean <- str_replace(raw_title, "^\\d+(\\.\\d+)?(\\.[a-z])?\\s*", "")
    item_label <- stringr::str_wrap(item_label_clean, width = 50)
  ...
  # 2. Generar Heatmap
  cat("\n\n---\n\n")
  cat("## Mapa de Calor: Distribución del Curso\n\n")
  ...
  g <- ggplot(all_data, aes(x = level, y = item, fill = prop)) +
    ...
    theme(
      ...
      # EJE Y (Texto de los ítems) - ALINEACIÓN DERECHA (hjust = 1)
      axis.text.y = element_text(
        size = 14, 
        color = "#333333", 
        margin = margin(r=10), 
        lineheight = 1.1, 
        hjust = 1  # <--- ESTO ALINEA EL TEXTO A LA DERECHA
      ),
      
      # TÍTULOS DE COMPETENCIA
      strip.text.y.left = element_text(angle = 0, face = "bold", size = 14, color = "#0a4e58"),
      strip.placement = "outside",
      strip.background = element_rect(fill = "#f0f0f0", color = NA),
