...
parse_deep_excel <- function(path){
  ...
  # Extraer filas de títulos (Fila detectada y la siguiente)
...
::: {.div style="font-size: 45px; text-align: left; margin-top: 3em;"}
...
'), "\n\n")
    ...
    # 1. Base válida (excluyendo NR)
    df_valid <- df %>% filter(level %in% c("0","1","2","3"))
    total_valid <- sum(df_valid$n)
    
    # 2. Índice de Calidad (0 a 100): (Puntaje Real / Puntaje Ideal) * 100
    puntos_reales <- sum(as.numeric(as.character(df_valid$level)) * df_valid$n)
    puntos_ideales <- total_valid * 3
    indice_calidad <- if(puntos_ideales > 0) (puntos_reales / puntos_ideales) else 0
    
    # 3. Logro Efectivo (Niveles 2 + 3)
    pct_logro <- sum(df$prop[df$level %in% c("2","3")])
    
    # 4. Brecha Crítica (Niveles 0 + 1)
    ...
    g <- ggplot(df, aes(x = level, y = n)) +
      ...
      theme(
        ...
        axis.text.x = element_text(size = 15, color = brand_text, lineheight = 1.1, margin = margin(t=2)),
    ...
    # Caja de Análisis con Indicadores de Valor
    cat('<div class="analysis-box">')
      cat('<h4 style="margin-top:5px; color:#0a6d7c; border-bottom:1px solid #ccc;">Indicadores Clave</h4>')
      
      # Indicador 1: Índice de Calidad (Estilo Semáforo)
      color_ic <- if(indice_calidad >= 0.7) "#28a745" else if(indice_calidad >= 0.5) "#ffc107" else "#dc3545"
      ...
      cat(glue::glue('<strong style="font-size:1.1em; color:{color_ic};">{scales::percent(indice_calidad, accuracy=0.1)}</strong>'))
      cat('</div>')
      
      # Indicador 2: Logro vs Brecha
    ...
    # Pie de página explicativo (Pequeño)
...
if (nrow(final_items) > 0) {
  ...
  for(i in 1:nrow(final_items)){
      ...
      g <- ggplot(df, aes(x = level, y = prop)) +
        ...
        labs(x=NULL, y=NULL) +
        theme(
           axis.text.x = element_text(size = 11, color = brand_text, lineheight = 1.1),
